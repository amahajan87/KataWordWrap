...
public class Wrapper {
	 ...
	 * correctly breaks the line.
	 * <li>If there is a space at the column index, then append only the characters
	 * till the index of the last space and continue calling the same method with
	 * remaining line starting from the character after the space`
	 * <li>
	 * <li>If there is a space, then identify the last space, break the line till
	 * before the space and continue calling the same method with remaining line
	 * starting from the character after the space
	 * <li>
	 * <li>If there is no space, then continue calling the same method with
	 * remaining line starting from the index after the column number.
	 * <li>
	 * 
	 * The function terminates when the input string's length is less than equal to
	 * the column number
	 * 
	 * @param inputString
	 * @param columnNumber
	...
	public static String wrap(final String inputString, final int columnNumber) {
		...
		// If the column number is not greater then equal to 1, then set the column
		// number as 1
		if (columnNumber < 1) {
			columnIndex = 1;
		}
